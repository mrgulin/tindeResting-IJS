file = open("Mzinput/BPF_mz_Strict.csv", "r")
...
for i in range(1, len(text3)):
    ...
    index1 = (d2["BPF-A1"][1] + d2["BPF-A2"][1]) / (d4["BLANK-A"] * 2 + 0.001)
    index1_1 = (d2["BPF-A1"][1] + d2["BPF-A2"][1]) / (2 * d4["BPF-B"] + 0.001)
    index2 = (d2["BPF-A1"][1] + d2["BPF-A2"][1]) / (d4["BPF-A1"] + d4["BPF-A2"] + 0.001)
    index3 = (d2["BPF-A1"][1] + d2["BPF-A2"][1]) / 2
    index4 = (float(text3[i][text3[0].index("BPF-A1_0")]) + float(text3[i][text3[0].index("BPF-A2_0")])) / 2
    index4_1 = (text3[i][text3[0].index("BPF-A1_0")] + text3[i][text3[0].index("BPF-A2_0")]) / (
                d4["BPF-A1"] + d4["BPF-A2"] + 0.001)
    text3[i][3] = index1
    text3[i][6] = index2
    text3[i][7] = index3
    text3[i][2] = index4
